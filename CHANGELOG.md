...
- When a unit gains a veterancy rank from a kill, it now gets a small reward beat — a
  ...
  earned instead of silent. Fires from both melee/ranged and splash kills. Verified:
  a footman's rank-crossing kill emits the flourish.
...
- Added a skirmish **difficulty** selector to the pause menu (Game tab). It scales
  ...
  (muster threshold +3 / 0 / −2): **Recruit** builds slower and waves later/rarer;
  **Warlord** rushes harder and sooner. Persisted to localStorage, applied live and
  to new games. Verified: selector renders + hit-tests, and Hard sends its first
  wave by ~90s vs ~120s on Soldier.
...
- The top resource bar now briefly **flashes red** when an action is denied
  ...
  block reads instantly. Decays over ~0.45s.
...
- Fixed command keys double-firing with WASD camera panning: every command-card
  ...
  Footman F, Archer R (was A), Knight T, Catapult C, Mage G, Dragon V (was D),
  Peon Q; Stop → X (was S). Hotkeys now cluster on the left hand around WASD.
